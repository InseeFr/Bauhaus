import { expect, test } from '@playwright/test';

import { uniqueId, uniqueLabel } from '../helpers/data';
import { clearChips, selectFirstMany, selectFirstOne, selectOne } from '../helpers/prime';

/**
 * Parcours n°8 : créer une composante mutualisée, brique de base des
 * structures de données (DSD).
 */
test('crée une composante mutualisée et la retrouve dans la liste', async ({ page }) => {
	const notation = uniqueId('COMP');
	const labelLg1 = uniqueLabel('Composante');

	await page.goto('/structures/components');
	await expect(page.getByRole('heading', { name: 'Components - Search' })).toBeVisible();
	await page.getByRole('link', { name: 'New' }).click();

	await page.getByLabel('Notation*').fill(notation);
	await page.getByLabel('Libellé*').fill(labelLg1);
	await page.getByLabel('Label*', { exact: true }).fill(`${labelLg1} EN`);
	await selectOne(page, 'Type', 'Attribute');
	await selectFirstOne(page, 'Owner');

	// Le contributeur pré-rempli est un timbre, que le back refuse : on le
	// remplace par une organisation du référentiel (cf. test `fixme` ci-dessous).
	await clearChips(page, 'Contributors');
	await selectFirstMany(page, 'Contributors');

	await page.getByRole('button', { name: 'Save' }).click();

	await expect(page).toHaveURL(/\/structures\/components\/[^/]+$/);
	await expect(page.getByRole('heading', { name: labelLg1 })).toBeVisible();

	await page.goto('/structures/components');
	await page.getByRole('textbox', { name: 'Search...' }).fill(labelLg1);
	await expect(page.getByRole('link', { name: labelLg1 })).toBeVisible();
});

/**
 * Régression connue : le formulaire pré-remplit « Contributeurs » avec le
 * timbre de l'utilisateur (`DG75-H250`), et POST /api/structures/components
 * échoue sur `IllegalArgumentException: Not a valid (absolute) IRI: DG75-H250`
 * (StructureComponentUtils.createRDFForComponent). Autrement dit, le formulaire
 * tel qu'il est proposé à l'utilisateur ne peut pas être enregistré sans
 * modifier le champ. Même symptôme sur la création de liste de codes.
 * À réactiver quand le contrat timbre / IRI sera aligné.
 */
test.fixme('enregistre une composante avec le contributeur pré-rempli', async ({ page }) => {
	const notation = uniqueId('COMP');
	const labelLg1 = uniqueLabel('Composante');

	await page.goto('/structures/components/create');
	await page.getByLabel('Notation*').fill(notation);
	await page.getByLabel('Libellé*').fill(labelLg1);
	await page.getByLabel('Label*', { exact: true }).fill(`${labelLg1} EN`);
	await selectOne(page, 'Type', 'Attribute');
	await selectFirstOne(page, 'Owner');

	await page.getByRole('button', { name: 'Save' }).click();

	await expect(page).toHaveURL(/\/structures\/components\/[^/]+$/);
});
