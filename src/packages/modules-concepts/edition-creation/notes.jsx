...
const handleFieldChange = (handleChange, maxLengthScopeNote) =>
  noteTypes(maxLengthScopeNote).reduce((handlers, { noteLg1Name, noteLg2Name }) => {
    ...
    <TabView>
      {noteTypes(maxLengthScopeNote).map(({ rawTitle, noteLg1Name, noteLg2Name, maxLength }, i) => {
        ...
        const highlight =
          ...
          (noteLg1Name === "scopeNoteLg1" &&
            (htmlLength(noteLg1) > maxLength || htmlLength(noteLg2) > maxLength));
        const title = highlight ? <div className="red">{D1[rawTitle]}</div> : D1[rawTitle];
