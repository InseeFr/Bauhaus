...
interface DocumentsBlocTypes {
  documents?: Document[];
  localPrefix?: "Lg1" | "Lg2";
  editMode?: boolean;
  deleteHandler?: (uri?: string) => void;
  addHandler?: (document: Document) => void;
  onReorder?: (activeId: string, overId: string) => void;
  objectType: "documents" | "links";
  idMas?: string;
}
...
export function DocumentsBloc({
  ...
  const defaultBtnBlocFunction = (document: Document) => (
    <button
      type="button"
  ...
  const currentDocumentsIds = currentDocuments.map((doc) => doc.uri) as string[];

  const labelField = `label${localPrefix}` as "labelLg1" | "labelLg2";

  const otherDocuments = documentStores[localPrefix.toLowerCase() as "lg1" | "lg2"]
    .filter((document) => !currentDocumentsIds.includes(document.uri ?? ""))
    .filter((document) => !!document[labelField])
    .filter((document) => (objectType === "documents" ? isDocument(document) : isLink(document)))
    .filter((document) =>
      (document[labelField] ?? "").toLowerCase().includes(filter.toLowerCase()),
    );
  ...
  function displayHTMLForDocument(
    document: Document,
    btnBlocFunction: (document: Document) => ReactNode = defaultBtnBlocFunction,
  ) {
    return (
          ...
          <div className="panel-heading">
            ...
            <button
              ...
              onClick={() => {
                ...
                setRubricIdForNewDocument({ rubric: idMas as string, lang: localPrefix });
              }}
            >
              <AddLogo />
            </button>
          ...
          {panelStatus && (
            <div className="panel-body">
              ...
              <ul className="documentsbloc-filepicker">
                {otherDocuments
                  .filter((_: Document, index: number) => index < 100)
                  .map((document) => {
                    return displayHTMLForDocument(document, (document: Document) => (
                      <button
                        type="button"
                        className="documentsbloc-delete documentsbloc-btn"
                        aria-label={t("app.btnAdd")}
