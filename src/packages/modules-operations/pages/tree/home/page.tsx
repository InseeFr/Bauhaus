...
const findNodeByKey = (nodes: TreeNode[], key: TreeNode["key"]): TreeNode | null => {
...
export const Component = () => {
  ...
  useEffect(() => {
    OperationsApi.getAllFamilies().then((data: FamilyItem[]) => {
  ...
  const onExpand = (event: TreeEventNodeEvent) => {
    ...
    if (nodeData.type === "family") {
      OperationsApi.getFamilyById(nodeData.id).then(
      ...
      OperationsApi.getSerie(nodeData.id).then(
        ({ operations = [] }: { operations?: OperationItem[] }) => {
          const updatedTreeData = [...treeData];
          const seriesNode = findNodeByKey(updatedTreeData, node.key);
          if (seriesNode) {
            seriesNode.children = operations.map((o) => formatOperation(o, nodeData.id));
          }
          setTreeData(updatedTreeData);
          setLoadingNodes((prev) => {
            const newSet = new Set(prev);
