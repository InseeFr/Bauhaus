...
const SearchFormList = ({ concepts, data }: Readonly<SearchFormListTypes>) => {
  ...
  const dataLinks = filteredData.map((component: ComponentModel) => (
...
export const Component = () => {
  ...
  useEffect(() => {
    Promise.all([StructureApi.getMutualizedComponentsForSearch(), ConceptsApi.getConceptList()])
      .then(([components, concepts]: [ComponentModel[], any[]]) => {
        setItems(components);
