...
interface MutualizedComponentsSelectorTypes {
  ...
  concepts: any;
  codelists: Codelists;
  handleCodelistDetail: (codelist: any) => void;
  readOnly?: boolean;
}
...
export const MutualizedComponentsSelector = ({
...
}: Readonly<MutualizedComponentsSelectorTypes>) => {
  ...
  const componentsWithActions = components.map((component) => ({
    ...
    concept: concepts.find(({ id }: any) => component.concept?.toString().includes(id?.toString()))
      ?.label,
    representation: (
      <Representation
        component={component}
