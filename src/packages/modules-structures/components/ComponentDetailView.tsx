...
interface ComponentDetailViewTypes {
  component: Component;
  concepts?: any[];
  codelists?: Codelists;
...
export const ComponentDetailView = ({
  ...
  serverSideError,
  attributes,
}: Readonly<ComponentDetailViewTypes>) => {
  ...
  const [partialCodelists, setPartialCodelists] = useState<PartialCodelist[]>([]);
    ...
    <>
        ...
        <Note
          text={
            <ul>
              ...
              <li>
                {t("component.creator")} : <InseeOrganization creator={component.creator ?? null} />
              </li>
              <li>
      ...
      {mutualized && component.structures?.length > 0 && (
        <Row>
          <Note
            text={
              <ul>
                {/* Populated as full structure references ({ id, labelLg1 }), not the plain
                    string iris the `Component.structures` model type otherwise declares. */}
                {(component.structures as unknown as { id: string; labelLg1: string }[])?.map(
                  (structure) => {
                    return (
                      <li key={structure.id}>
      ...
      <CodelistPanel
        codelist={fullCodelists.find(
          (c) =>
            ((component.codeList as any)?.id || component.codeList)?.toString() ===
            c.id?.toString(),
        )}
