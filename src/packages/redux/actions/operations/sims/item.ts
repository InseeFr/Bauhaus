...
function getFetchLabelsPromise(sims: any, promise: any) {
  ...
  if (sims.idOperation) {
    return OperationsApi.getOperation(sims.idOperation).then((parent: any) => mergeLabels(parent));
  }
  if (sims.idSeries) {
    return OperationsApi.getSerie(sims.idSeries).then((parent: any) => mergeLabels(parent));
  }
  if (sims.idIndicator) {
    return OperationsApi.getIndicatorById(sims.idIndicator).then((parent: any) =>
      mergeLabels(parent),
    );
  }
  return promise;
...
const fetchSims = (id: string) => (dispatch: any, getState: any) => {
  ...
  return OperationsApi.getSims(id).then(
    (results: any) => {
      return getParentsWithoutSims(results.idOperation).then((parentsWithoutSims = []) => {
