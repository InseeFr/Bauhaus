...
export const LOAD_OPERATIONS_METADATASTRUCTURE_LIST = "LOAD_OPERATIONS_METADATASTRUCTURE_LIST";
...
const fetchMsd = () => (dispatch: Dispatch, getState: any) => {
  ...
  return Promise.all([
    ...
    ([metaDataStructure, metadataAttributes]: [any, any]) => {
      const metadataAttributesObject = metadataAttributes.reduce((acc: any, attr: any) => {
        return {
          ...acc,
          [attr.id]: {
            ...attr,
      ...
      const metaDataStructureTree = getTree(metaDataStructure, undefined, metadataAttributesObject);
      const codesList = new Set();
      metadataAttributes
        .filter((attr: any) => attr.codeList)
        .map((attr: any) => attr.codeList)
        .filter(
          (code: any) => Object.keys(getState().operationsCodesList.results).indexOf(code) < 0,
        )
