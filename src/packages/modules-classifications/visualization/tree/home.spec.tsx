...
describe("ClassificationTree", () => {
  ...
  it("renders empty state when no data", () => {
    const { container } = renderWithAppContext(<ClassificationTree {...defaultProps} data={[]} />);
  ...
  it("sorts tree nodes alphabetically", () => {
    ...
    const { container } = renderWithAppContext(
      <ClassificationTree data={unsortedData} prefLabel="Test" secondLang={false} />,
    );
  ...
  it("generates correct links for tree nodes", () => {
    ...
    links.forEach((link) => {
      ...
      expect(href).toMatch(
        /^\/classifications\/classification\/test-classification-id\/item\/\d+$/,
      );
    });
  });
  ...
  it("handles duplicate items correctly", () => {
    ...
    const { container } = renderWithAppContext(
      <ClassificationTree data={dataWithDuplicates} prefLabel="Test" secondLang={false} />,
    );
  ...
  it("handles empty children arrays correctly", () => {
    ...
    const { container } = renderWithAppContext(
      <ClassificationTree data={dataWithEmptyChildren} prefLabel="Test" secondLang={false} />,
    );
  ...
  it("processes parent-child relationships correctly", () => {
    ...
    const { container } = renderWithAppContext(
      <ClassificationTree data={complexData} prefLabel="Test" secondLang={false} />,
    );
