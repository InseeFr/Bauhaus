...
vi.mock("react-i18next", async () =>
  (await import("../representation.testing")).mockTranslations({
    ...
    "physicalInstance.view.sentinel.usersPanel.title": "Utilisations de ces valeurs sentinelles",
...
/** MMVR matérialisée localement (création à la volée ou édition en cours). */
const localMmvr = {
  $type: "ManagedMissingValuesRepresentation",
  ID: "mmvr-1",
  Agency: "fr.insee",
...
describe("SentinelValues", () => {
  ...
  /** Idem, avec la MMVR et sa liste de codes portées par des modifications locales en cours. */
  const renderWithLocalEdits = (props: Partial<ComponentProps<typeof SentinelValues>> = {}) =>
    renderWithReference({
      mmvr: localMmvr,
  ...
  const mockLoadedCodeListUsedBy = (...variableIds: string[]) => {
    mockUseMutualizedCodeList.mockReturnValue({ isLoading: false, data: sentinelCodeListContent });
    mockUseMmvrUsers.mockReturnValue({ data: variableIds.map(usage), isLoading: false });
  };
  ...
  it("shows the code list as read-only when at least one OTHER variable uses the MMVR", () => {
    ...
    mockLoadedCodeListUsedBy("var-1", "var-2");
  ...
  it("keeps the code list editable and materializes edits under the same IDs when only this variable uses the MMVR", () => {
    const onChange = vi.fn();
    mockLoadedCodeListUsedBy("var-1");

    renderWithReference({ onChange });
  ...
  it("locks the code list when another UNSAVED local variable references the same MMVR", () => {
    ...
    renderWithReference({ locallyUsedMmvrIds: ["mmvr-1"] });
