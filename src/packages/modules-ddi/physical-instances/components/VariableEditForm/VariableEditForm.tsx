...
type FormAction =
	...
	| { type: 'SET_NUMERIC_REPRESENTATION'; payload: NumericRepresentation | undefined }
	| { type: 'SET_DATE_REPRESENTATION'; payload: DateTimeRepresentation | undefined }
	| { type: 'SET_TEXT_REPRESENTATION'; payload: TextRepresentation | undefined }
	| { type: 'SET_CODE_REPRESENTATION'; payload: { codeRep: CodeRepresentation | undefined; codeList?: CodeList; categories?: Category[] } }
...
function formReducer(state: FormState, action: FormAction): FormState {
	switch (action.type) {
		...
		case 'SET_NUMERIC_REPRESENTATION':
			return {
				...state,
				representation: { ...state.representation, NumericRepresentation: action.payload },
			};
		case 'SET_DATE_REPRESENTATION':
			return {
				...state,
				representation: { ...state.representation, DateTimeRepresentation: action.payload },
			};
		case 'SET_TEXT_REPRESENTATION':
			return {
				...state,
				representation: { ...state.representation, TextRepresentation: action.payload },
			};
...
export const VariableEditForm = ({
	...
	const updateNumericRepresentation = useCallback((numericRep: NumericRepresentation | undefined) => {
	...
	const updateCodeRepresentation = useCallback((codeRep: CodeRepresentation | undefined, codeList?: CodeList, categories?: Category[]) => {
		dispatch({ type: 'SET_CODE_REPRESENTATION', payload: { codeRep, codeList, categories } });
	}, []);
	...
	const handleSubmit = useCallback(
		(e: React.FormEvent<HTMLFormElement>) => {
			...
			onSave({
				...
				numericRepresentation:
					state.selectedType === 'numeric' ? state.representation.NumericRepresentation : undefined,
				dateRepresentation:
					state.selectedType === 'date' ? state.representation.DateTimeRepresentation : undefined,
				textRepresentation:
					state.selectedType === 'text' ? state.representation.TextRepresentation : undefined,
				codeRepresentation:
					state.selectedType === 'code' ? state.representation.CodeRepresentation : undefined,
				codeList: state.selectedType === 'code' ? state.representation.CodeList : undefined,
				categories: state.selectedType === 'code' ? state.representation.Category : undefined,
			});
		},
			...
			<form onSubmit={handleSubmit} className="flex flex-column gap-3">
				...
				<div className="flex flex-column gap-2">
					...
					<Dropdown
						...
						onChange={(e) => dispatch({ type: 'SET_TYPE', payload: e.value })}
						options={typeOptions}
				...
				<div className="flex align-items-center gap-2">
					<Checkbox
						...
						onChange={(e) => dispatch({ type: 'SET_IS_GEOGRAPHIC', payload: e.checked ?? false })}
					/>
