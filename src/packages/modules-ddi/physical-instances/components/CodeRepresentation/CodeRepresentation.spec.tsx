...
describe('CodeRepresentation', () => {
	...
	it('should display codes list options in dropdown', () => {
		...
		expect(options.some((opt) => opt.textContent === 'Liste des statuts professionnels')).toBe(true);
	...
	it('should format dropdown value as agency-id', () => {
		...
		const secondOption = Array.from(options).find(
			(opt) => opt.textContent === 'Liste des pays',
		);
