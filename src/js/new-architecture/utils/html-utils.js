...
import { draftjsToMd } from '../../utils/utils/draftjs/draftjsToMd';
import { mdToDraftjs, REGEXPS } from '../../utils/utils/draftjs/mdToDraftjs';
	...
	for (let blockIndex = 0; blockIndex < content.blocks.length; blockIndex++) {
		...
		for (
			...
			let minLength = Math.min(
				...
				...withSameOffset.map((style) => style.length)
			);
