            ...
            <ul>
              { Object.keys(mapping).map(fieldName => {
                if (attr.hasOwnProperty(fieldName)) {
                  if (fieldName === 'altLabelEn' && !english) {return null;}
                  if (fieldName === 'created' || fieldName === 'modified' || fieldName === 'dateEnd') {
                    return <li key={fieldName}>{`${mapping[fieldName]} : ${dateTimeToDateString(attr[fieldName])}`}</li>
                  }
                  if (fieldName === 'additionnalMaterial') {
                    return <li key={fieldName}>{`${mapping[fieldName]} : `}
                      <a href={attr[fieldName]} target="_blank">{`${attr[fieldName]}`}</a></li>
                  }
                  else {
