...
class CollectionsToValidate extends Component {
  ...
  constructor(props) {
    ...
    this.OnClickAddMember = (e) => {
      this.setState({
        ...
        potentialCollectionsToValid:  _.pull(this.state.potentialCollectionsToValid, e)
      });
    }
    this.OnClickDelMember = (e) => {
      this.setState({
        collectionsToValid: _.pull(this.state.collectionsToValid, e),
        potentialCollectionsToValid: [...this.state.potentialCollectionsToValid, e]
      });
    }
  ...
  render() {
    const { searchLabel, potentialCollectionsToValid, collectionsToValid, validation } = this.state
    ...
    const itemsList = sortByLabel(
        potentialCollectionsToValid.filter(filterByPrefLabelFr(_.deburr(searchLabel))))
        .map((item) =>
        ...
        <div className="container">
          <div className="row">
            <div className='col-md-10 centered col-md-offset-1'>
              <h2 className="page-title">{dictionary.collections.validation.title}</h2>
            </div>
          </div>
          ...
          {collectionsToValid.length === 0 &&
              <div className='row btn-line'>
                ...
                <div className='col-md-2'>
                  <button className="btn btn-primary btn-lg col-md-12 pull-right" onClick={this.handleClickModif} disabled>
                    {dictionary.buttons.validate}
                  </button>
                </div>
              </div>}
