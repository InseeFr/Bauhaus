...
import { sortArray, filterByPrefLabelFr, filterByDefinitionFr, filterByCreator,
   filterByDisseminationStatus, filterByValidationStatus, filterByCreatedDate, filterByModifiedDate } from '../utils/array-utils';
...
class ConceptsSearchList extends Component {
  ...
  render() {
    const { conceptsSearchList, stampsList, disseminationStatusList } = this.props
    const { searchLabel, searchDef, creator, disseminationStatus, validationStatus,
          dateStartCreate, dateEndCreate, dateStartModify, dateEndModify} = this.state

          console.log(this.state)

    if (conceptsSearchList.length === 0) return (
      <div>
        <MenuConcepts />
        <Loadable active={true} spinner text={dictionary.loadable.loading} color='#457DBB' background='grey' spinnerSize='400px' />
      </div>
    );
    const itemsList = sortByLabel(
        ...
        <li key={item.id} className="list-group-item"><Link to={'/concept/'+item.id}>{item.prefLabelFr}</Link></li>
    )
        ...
        <div className='container'>
          ...
          <div className="row btn-line">
            <div className='col-md-2'>
              <button type="button" className="btn btn-primary btn-lg col-md-12" onClick={this.onClickReturn}>
                <span className="glyphicon glyphicon-menu-left" aria-hidden="true">
                </span> {dictionary.buttons.return}
              </button>
            </div>
            <div className='col-md-2 pull-right'>
              <button type="button" className="btn btn-primary btn-lg col-md-12" onClick={this.onClickInitialize}>
                <span className="glyphicon glyphicon-flash" aria-hidden="true">
                </span> {dictionary.buttons.initialize}
              </button>
            </div>
          </div>
          <div className="row form-group">
            <div className="col-md-12">
              <input value={searchLabel} onChange={e => this.handleChangeSearchLabel(e.target.value)}
                type='text' placeholder={dictionary.concepts.search.label} className='form-control'/>
            </div>
          </div>
          <div className="row form-group">
            <div className="col-md-12">
              <input value={searchDef} onChange={e => this.handleChangeSearchDef(e.target.value)}
                type='text' placeholder={dictionary.concepts.search.definition} className='form-control'/>
            </div>
          </div>
          <div className="row form-group">
            ...
            <div className='col-md-4'>
              { disseminationStatusList.length>0 &&
                  <SelectRmes
                    className="form-control"
                    placeholder={dictionary.status.concept.validationStatus.defaultValue}
          ...
          <div className='row vertical-center'>
            ...
            <div className='col-md-4'>
              <DatePickerRmes value={dateStartCreate} onChange={this.handleChangeStartCreate} placement='bottom' />
            </div>
            <div className='col-md-1 centered'>
              <label> et le </label>
            </div>
            <div className='col-md-4'>
              <DatePickerRmes value={dateEndCreate} onChange={this.handleChangeEndCreate} placement='bottom' />
            </div>
          </div>
          <div className='row vertical-center'>
            <div className='col-md-3 centered'>
            ...
            <div className='col-md-4'>
              <DatePickerRmes value={dateStartModify} onChange={this.handleChangeStartModify} placement='bottom' />
            </div>
            <div className='col-md-1 centered'>
              <label> et le </label>
            </div>
            <div className='col-md-4'>
              <DatePickerRmes value={dateEndModify} onChange={this.handleChangeEndModify} placement='bottom' />
            </div>
          </div>
