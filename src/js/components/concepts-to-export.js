...
class ConceptsToExport extends Component {
  ...
  constructor(props) {
    ...
    this.OnClickAddMember = (e) => {
      this.setState({
        ...
        potentialConceptsToExport:  _.pull(this.state.potentialConceptsToExport, e)
      });
    }
    ...
    this.handleClickExport = e => {
      ...
      postConceptsToExport(data)
        .then(res => res.blob())
        .then((blob) => {return FileSaver.saveAs(blob, `concepts.ods`);})
        .then(() => {
          this.setState({
            validation: 'DONE'
          })
  ...
  render() {
    const { searchLabel, potentialConceptsToExport, conceptsToExport, validation } = this.state
    ...
    const itemsList = sortByLabel(
        potentialConceptsToExport.filter(filterByPrefLabelFr(_.deburr(searchLabel))))
        .map((item) =>
        ...
        <div className="container">
          <div className="row">
            <div className='col-md-10 centered col-md-offset-1'>
              <h2 className="page-title">{dictionary.concepts.export.title}</h2>
            </div>
          </div>
          ...
          <div className='row'>
            ...
            <div className='col-md-6 centered'>
              <input value={searchLabel} onChange={e => this.handleChange(e.target.value)} type='text' placeholder={dictionary.concepts.searchLabel} className='form-control'/>
...
export default connect(mapStateToProps) (ConceptsToExport)
