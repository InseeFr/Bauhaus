...
import { loadCollectionsList, loadCollectionsToValidateList } from '../actions/collections-list';
...
class CollectionsList extends Component {
  ...
  render() {
    ...
    const itemsList = sortByLabel(
      ...
      .map((item) =>
      <li key={item.id} className="list-group-item"><Link to={'/collection/'+item.id}>{item.prefLabelFr}</Link></li>
    )
    return (
      <div>
