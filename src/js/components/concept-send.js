...
class ConceptSend extends Component {
  ...
  constructor(props) {
    ...
    var params = [this.props.conceptGeneral.prefLabelFr, this.props.conceptGeneral.id]
    ...
    this.state = {
      ...
      object: dictionary.concept.send.object.value([this.props.conceptGeneral.prefLabelFr]),
    ...
    this.handleChangeRec = recipient => {
      ...
      if (recipient.endsWith('@insee.fr')) {
        ...
        this.setState({ message: EditorState.createWithContent(stateFromHTML(message)) })
      }
    ...
    this.handleClickSend = e => {
      ...
      const data = {
        ...
        conceptsToExport: [{"id": this.props.conceptGeneral.id, "prefLabelFr" :this.props.conceptGeneral.prefLabelFr}],
        prefLabelFr: this.props.conceptGeneral.prefLabelFr
      }
      this.setState({
        creation: 'PENDING'
      })
      postConceptSend(data)
        .then(isSent =>
          isSent.text()
        )
        .then(isSent => {
  ...
  render() {
    ...
    if (creation === 'PENDING') {
      return (
        <div>
          <MenuConcepts />
          <Loadable active={true} spinner text={dictionary.loadable.sending} color='#457DBB' background='grey' spinnerSize='400px' />
        </div>
      )
    }

    if (creation === 'DONE' || creation === 'FAILED') {
      const onClick = creation === 'DONE' ? this.handleClickReturn : this.handleClickReturnFailed
      return (
        <div>
          <MenuConcepts />
          <div className='container'>
            <div className='row centered'>
              <div className='col-md-12'>
                {creation === 'DONE' && <h2>{dictionary.concept.send.success([conceptGeneral.prefLabelFr])}</h2>}
                {creation === 'FAILED' && <h2>{dictionary.concept.send.failed([conceptGeneral.prefLabelFr])}</h2>}
              </div>
            </div>
            <div className='row'>
              <div className='col-md-12'>
                <button className='btn btn-primary btn-lg col-md-2 col-md-offset-5' onClick={onClick}>
    ...
    return (
      <div>
        ...
        <div className='container'>
          ...
          <SendControl attr={this.state} onChange={this.handleClickSend} onChangeReturn={this.handleClickReturn}/>
          <div className="form-group">
            <label>{dictionary.concept.send.recipient}</label>
            <input type="email" className="form-control" onChange={e => this.handleChangeRec(e.target.value)} />
          </div>
          <div className="form-group">
            <label>{dictionary.concept.send.sender}</label>
            <input type="text" className="form-control" defaultValue={sender} disabled/>
          </div>
          <div className="form-group">
            <label>{dictionary.concept.send.object.title}</label>
            <input type="text" className="form-control" defaultValue={object} onChange={e => this.handleChangeObj(e.target.value)}/>
          </div>
          <div className="form-group">
            <label>{dictionary.concept.send.message.title}</label>
            <EditorHtml editor={message} onEditorChange={e => this.changeMessage(e)} />
          </div>
        </div>
      </div>
    )
