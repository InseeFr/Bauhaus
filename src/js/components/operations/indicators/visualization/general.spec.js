...
describe('IndicatorInformation', () => {
	...
	it('should show the right data in the DisplayLinks component', () => {
		...
		const contributors = displayLinks.get(0);
		expect(contributors.props.links).toEqual([
			{
				id: 'CNAMTS',
				label: 'Agence centrale des organismes de sécurité sociale',
			},
		]);
		expect(contributors.props.path).toBeUndefined();
		expect(contributors.props.displayLink).toBeFalsy();
