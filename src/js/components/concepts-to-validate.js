...
class ConceptsToValidate extends Component {
  ...
  render() {
    const { searchLabel, potentialConceptsToValid, conceptsToValid, validation } = this.state
    ...
    const itemsList = sortByLabel(
        potentialConceptsToValid.filter(filterByPrefLabelFr(_.deburr(searchLabel))))
        .map((item) =>
      ...
      <div>
        ...
        <div className="row">
          <div className='col-md-10 centered col-md-offset-1'>
            <h2 className="page-title">{dictionary.concepts.validation.title}</h2>
          </div>
        </div>
          ...
          {conceptsToValid.length === 0 &&
              <div className='row btn-line'>
                ...
                <div className='col-md-2'>
                  <button className="btn btn-primary btn-lg col-md-12" onClick={this.handleClickModif} disabled>
                    {dictionary.buttons.validate}
                  </button>
                </div>
              </div>}
...
export default connect(mapStateToProps) (ConceptsToValidate)
