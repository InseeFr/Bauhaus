...
import { getConceptGeneral, getConceptNotes, getConceptLinks } from '../utils/remote-api'
import { transformArrayToKeepValues } from '../utils/array-utils'

export const loadConceptGeneral = (id) =>
  (dispatch, getState) => {
    dispatch({
    ...
    return getConceptGeneral(id)
      .then(conceptGeneral => {
            const data = transformArrayToKeepValues(conceptGeneral.results.bindings)[0]
...
export const loadConceptLinks = (id) =>
  (dispatch, getState) => {
    dispatch({
    ...
    return getConceptLinks(id)
      .then(conceptLinks => {
            dispatch(loadConceptLinksSuccess(id, transformArrayToKeepValues(conceptLinks.results.bindings)))
          })
...
export const loadConceptNotes = (id, conceptVersion) =>
  (dispatch, getState) => {
    ...
    return getConceptNotes(id, conceptVersion)
      .then(conceptNotes => dispatch(loadConceptNotesSuccess(id, conceptVersion, transformArrayToKeepValues(conceptNotes.results.bindings)[0])))
      .catch(err => dispatch(loadConceptNotesFailure(id, conceptVersion, err.toString())))
  }
...
export function loadConceptGeneralAndNotes(conceptId) {
  return (dispatch, getState) => {
    return dispatch(loadConceptGeneral(conceptId))
    .then(conceptGeneral => {
