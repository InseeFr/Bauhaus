...
export const transformArrayToKeepValues = rsltsArr =>
  rsltsArr.map(row => Object.keys(row).reduce((_, fieldName) => {
      _[fieldName] = row[fieldName].value
      return _
    }, {})
  )
...
export const arrayDropUniqueField = (array,fieldToRemove) =>
  array.map(row => Object.keys(row).reduce((obj, key) => {
...
export const arrayDifferenceByID = (array1, array2) => {
    ...
    const result = _.filter(array1, function(obj) { return diff.indexOf(obj.id) >= 0; })
    return result
}
...
export function filterByPrefLabelFr(filter) {
  return item => _.deburr(item.prefLabelFr).toLowerCase().includes(filter.toLowerCase())
}
export function filterByDefinitionFr(filter) {
  return item => _.deburr(item.definitionFr).toLowerCase().includes(filter.toLowerCase())
}
export function filterByCreator(filter) {
  return item => _.deburr(item.creator).toLowerCase().includes(filter.toLowerCase())
}
export function filterByDisseminationStatus(filter) {
  return item => _.deburr(item.disseminationStatus).toLowerCase().includes(filter.replace(' ','').toLowerCase())
}
export function filterByValidationStatus(filter) {
  return item => _.deburr(item.validationStatus).toLowerCase().includes(filter.toLowerCase())
}
...
export const creatSelectListSelectedLast = max => {
  ...
  result.push(<option value={max} key={max}>{max}</option>)
  return result
}
  ...
  for (var i = 0; i < array.length; i++) {
    if (attr) {
      if (array[i][attr] === init) {
        result.push(<option value={array[i][attr]} key={array[i][attr]} selected='selected'>{array[i][attr]}</option>)
      }
      else result.push(<option value={array[i][attr]} key={array[i][attr]}>{array[i][attr]}</option>)
    }
    else {
      if (array[i] === init) {
        result.push(<option value={array[i]} key={array[i]} selected='selected'>{array[i]}</option>)
      }
      else result.push(<option value={array[i]} key={array[i]}>{array[i]}</option>)
    }
  }
  return result
}
...
export const creatSelectListFromArrayWithInitText = (array, initText, attr) => {
  const result=[]
  result.push(<option value='-1' key='-1' selected='selected' disabled hidden>{initText}</option>)
  for (var i = 0; i < array.length; i++) {
    if (attr) {
      result.push(<option value={array[i][attr]} key={array[i][attr]}>{array[i][attr]}</option>)
    }
    else result.push(<option value={array[i]} key={array[i]}>{array[i]}</option>)
  }
  return result
...
export const getMembers = (linksArray, typeOfLink) => {
  ...
  for (var i = 0; i < linksArray.length; i++) {
    if(linksArray[i].conceptLink===typeOfLink) {
      result.push({id: linksArray[i].idLinked, prefLabelFr: linksArray[i].prefLabelFr})
    }
  }
  return result
}
