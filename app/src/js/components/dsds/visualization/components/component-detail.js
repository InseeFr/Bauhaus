...
import { Badge, PageTitle } from 'js/components/shared';
...
const ComponentDetails = ({ id, ...props }) => {
  ...
  const { type, labelLg1, labelLg2, attachment, concept, codeList, range } = component;

  if (!id) return <h3 className="centered empty-component">{D.emptyComponent}</h3>;
    ...
    <div>
      ...
      <ul>
        {C.ATTACHMENTS.find(a => a.value === attachment) && (
          <li>
            {D.attachmentTitle} : {C.ATTACHMENTS.find(a => a.value === attachment).label}
          </li>
        )}
