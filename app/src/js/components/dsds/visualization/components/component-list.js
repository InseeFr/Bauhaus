...
import { Input, Pagination, Badge } from 'js/components/shared';
...
const ComponentList = ({ components, checked, onCheck, onChange }) => {
  ...
  const items = buildComponents(checked, sortArrayByLabel(components), onChange, search);
    ...
    if (checked[type] && filterDeburr(search)(label)) {
      _.push(
        <div className="row" key={`${type}-${i}`}>
          <button onClick={() => onChange(id, type)} className="btn-no-btn list-group-item">
