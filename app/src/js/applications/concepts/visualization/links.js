...
import { BROADER, CLOSE_MATCH, IS_REPLACED_BY, NARROWER, REFERENCES, RELATED, SUCCEED } from 'js/constants';
import "./links.scss";

const CloseMatchLinks = ({ links, Dictionnary }) => {
	return links.length > 0 && (
		<li>{Dictionnary.equivalentTitle} :
			<ul> {
...
const LinksList = ({ links, lang, alone, Dictionnary = D1 }) => {
	...
	return (<Note
		text={
			<dl>
				<InternalLinks links={links[NARROWER]} title={Dictionnary.narrowerTitle} labelProperty={labelProperty} />
				<InternalLinks links={links[BROADER]} title={Dictionnary.broaderTitle} labelProperty={labelProperty} />
...
function ConceptLinks({ secondLang, links }) {
	const linksGroupByType = links.reduce((acc, link) => {
	...
	return (
		<div className='row concept-links'>
			...
			{secondLang && <LinksList links={linksGroupByType} alone={false} lang='lg2' Dictionnary={D2}/> }
		</div>
	);
}
