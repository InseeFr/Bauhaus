		...
		<div className='container'>
			...
			<Row>
				<Note
					text={
						<ul>
							<li>{D.createdDateTitle} : { DateUtils.stringToDate(distribution.created) } </li>
							<li>{D.modifiedDateTitle} : { DateUtils.stringToDate(distribution.updated) } </li>
							<li>{D.formatTitle} : { distribution.format } </li>
							<li>{D.tailleTitle} : { distribution.taille } </li>
							<li>{D.downloadUrlTitle} : <a target="_blank" rel="noreferrer noopener" href={distribution.url}>{ distribution.url } </a></li>
						</ul>
					}
