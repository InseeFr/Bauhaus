...
import {
	ADMIN, COLLECTIONS_CREATOR, CONCEPT_CONTRIBUTOR, CONCEPTS_CONTRIBUTOR, CONCEPTS_CREATOR, INDICATOR_CONTRIBUTOR, SERIES_CONTRIBUTOR
...
const options = [ADMIN, CONCEPTS_CREATOR, COLLECTIONS_CREATOR, CONCEPTS_CONTRIBUTOR, CONCEPT_CONTRIBUTOR, INDICATOR_CONTRIBUTOR, SERIES_CONTRIBUTOR].map(role => ({ value: role, label: role }));
...
function install(store) {
	...
	function AdminTool() {
		...
		const value = options.filter(({ value }) => rolesAndStamps.roles.indexOf(value) >= 0);
		return (
			<div>
				<label style={{ width: '100%'}}>
					{D.pickedRolePlaceholder}
					<Select options={options} value={value} multi={true} onChange={value => changeRoles(value)}/>
				</label>
			</div>
		)
	}
