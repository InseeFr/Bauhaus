...
ENTRYPOINT [ "sh", "-c", "/usr/share/nginx/html/vite-envs.sh && : "${VITE_API_BASE_HOST:?Set VITE_API_BASE_HOST env var}" && : "${VITE_OIDC_BASE_URL:?Set VITE_OIDC_BASE_URL env var}" && sed -e "s|__API_BASE_HOST__|${VITE_API_BASE_HOST}|g" -e "s|__OIDC_BASE_URL__|${VITE_OIDC_BASE_URL}|g" /usr/share/nginx/html/index.html > /tmp/index.html && cat /tmp/index.html > /usr/share/nginx/html/index.html && nginx -g 'daemon off;'" ]
